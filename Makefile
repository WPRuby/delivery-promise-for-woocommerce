...
zip:
	...
	mkdir -p "$(PACKAGE_DIR)/assets/admin/dist" "$(PACKAGE_DIR)/assets/frontend"
...
validate-build:
	...
	! unzip -l "$(ZIP_FILE)" | rg -q '$(PLUGIN_SLUG)/(tests|node_modules|\.git|\.github|vendor|build|dist|assets/admin/vue)/'
	! unzip -l "$(ZIP_FILE)" | rg -q '$(PLUGIN_SLUG)/(composer\.(json|lock)|package(-lock)?\.json|phpunit\.xml(\.dist)?|vite\.config\.|Makefile|\.env)'
